...
class MinimalSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Float64MultiArray, 'angle_topic', self.listener_callback, 10)
        self.num_node = 21
        self.num_fin = 5
        self.num_input = 15
        self.im_height = 640
        ...
        self.angle_0 = np.array([np.pi / 9, np.pi * 5 / 12, np.pi / 2, np.pi * 7 / 12, np.pi * 2 / 3])
    ...
    def listener_callback(self, msg_data):
        ...
        node_pixel = 100 * np.array([[1, 0, 0], [0, -1, 0], [0, 0, 1]]) @ node_rot_point + np.array([320*np.ones(self.num_node), 550*np.ones(self.num_node), np.zeros(self.num_node)])
