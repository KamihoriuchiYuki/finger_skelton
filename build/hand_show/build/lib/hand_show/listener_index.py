...
class MinimalSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.subscription = self.create_subscription(Float64MultiArray, '/predicted_output_topic', self.listener_callback, 10)
        self.num_node = 21
        self.num_fin = 5
        self.num_input = 15
        self.im_height = 1280
    ...
    def listener_callback(self, msg_data):
        ...
        angle[1] = np.pi / 2
        angle[2] = np.pi / 2
        angle[3] = angle_index[3]
        angle[4] = angle_index[4]
        angle[5] = angle_index[5]
        angle[6] = 5 * np.pi / 12
        angle[7] = np.pi / 2
        angle[8] = 5 * np.pi / 12
        angle[9] = 5 * np.pi / 12
