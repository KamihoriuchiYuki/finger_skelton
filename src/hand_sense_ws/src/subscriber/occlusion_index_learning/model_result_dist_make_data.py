...
class RsSub(Node):
    ...
    def toggle_recording(self):
        ...
        if self.recording:
            ...
            self.f = open(f"pre_learning_dist/index_finger_data_{timestamp}.csv", "w", newline="")
            self.writer = csv.writer(self.f)
            header = ["time[s]", "MCP", "PIP", "DIP", "TIP"]
            self.writer.writerow(header)

            self.pos_f = open(f"pre_learning_dist/finger_joint_positions_{timestamp}.csv", "w", newline="")
            self.pos_writer = csv.writer(self.pos_f)
            pos_header = ["time[s]", "MCP", "PIP", "DIP", "TIP", 
            ...
            self.video_out = cv2.VideoWriter(f"pre_learning_dist/index_finger_video_{timestamp}.avi", fourcc, 20.0, (640, 480))
    ...
    def plot_data(self):
        ...
        with open(self.f.name) as f_0:
            ...
            fig.savefig(f"pre_learning_dist/index_finger_distance_{time.strftime('%Y%m%d_%H%M%S')}.png")
            plt.close()
