...
class RsSub(Node):
    ...
    def listener_callback(self, msg):
        ...
        if self.recording:
            ...
            pos_row = [current_time] + [coord for pos in pixel_positions for coord in pos] + [coord for pos in joint_positions for coord in pos] + distance
    ...
    def toggle_recording(self):
        ...
        if self.recording:
            ...
            self.f = open(f"data_dist/thumb_data_{timestamp}.csv", "w", newline="")
            self.writer = csv.writer(self.f)
            header = ["time[s]", "MCP", "PIP", "DIP", "TIP"]
            self.writer.writerow(header)

            self.pos_f = open(f"data_dist/finger_joint_positions_{timestamp}.csv", "w", newline="")
            ...
            self.video_out = cv2.VideoWriter(f"data_dist/thumb_video_{timestamp}.avi", fourcc, 20.0, (640, 480))
    ...
    def plot_data(self):
        ...
        with open(self.f.name) as f_0:
            ...
            axs.set_title("Distance of Thumb Joints from Camera")
            fig.savefig(f"data_dist/thumb_distance_{time.strftime('%Y%m%d_%H%M%S')}.png")
            plt.close()
