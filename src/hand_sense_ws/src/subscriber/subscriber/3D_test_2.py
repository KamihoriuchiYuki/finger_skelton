...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # 手の幅、高さ、奥行きを計算
            hand_width = np.max(hand_points, axis=0)[0] - np.min(hand_points, axis=0)[0]
            hand_height = np.max(hand_points, axis=0)[1] - np.min(hand_points, axis=0)[1]
            hand_depth = np.max(hand_points, axis=0)[2] - np.min(hand_points, axis=0)[2]

            # 手のポイントを手の中心を基準にスケーリングして配置
            hand_points_scaled = []
            for point in hand_points:
                scaled_point = (point - hand_center) / np.array([hand_width, hand_height, hand_depth])
                hand_points_scaled.append(scaled_point)

            # 手のポイントクラウドを作成
            point_cloud = o3d.geometry.PointCloud()
            point_cloud.points = o3d.utility.Vector3dVector(hand_points_scaled)

            # 関節間の線分を描画
            lines = []
            for connection in connections:
                point1 = hand_points_scaled[connection[0]]
                point2 = hand_points_scaled[connection[1]]
                lines.append([point1, point2])
            line_set = o3d.geometry.LineSet(
                points=o3d.utility.Vector3dVector(hand_points_scaled),
