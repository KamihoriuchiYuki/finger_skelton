...
channel = 0
while True:
    print('Do you report angles of fingers?(y/n)')
    mode = input()
    if mode == 'y':
        channel = 0
        print('y')
...
class RsSub(Node):
    ...
    def listener_callback(self, msg):
        ...
        hand_image, index_finger = self.hand_skelton(array_rgb, num_node)
        pose_image, wrist = self.pose_skelton(array_rgb)
        ...
        print(wrist)
        image = array_rgb + hand_image + pose_image
    ...
    def pose_skelton(self, image):
        image_width, image_height = image.shape[1], image.shape[0]
        wrist = np.zeros((1, 2), dtype=np.int64)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        before_image = image.copy()
        ...
        if results.pose_landmarks:
            ...
            wrist[0, 0] = int(results.pose_landmarks.landmark[16].x * image_width)
            wrist[0, 1] = int(results.pose_landmarks.landmark[16].y * image_height)
