...
def autoencoder(input_dim, latent_dim, timesteps):
    # encode
    inputs_x = Input(shape=(timesteps, input_dim))
    encoded_x = Dense(latent_dim, activation="relu")(inputs_x)
    encoded_x = Dropout(0.2)(encoded_x)

    inputs_y = Input(shape=(timesteps, input_dim))
    encoded_y = Dense(latent_dim, activation="tanh")(inputs_y)
    ...
    decoded_x = Dropout(0.2)(hidden_x)
    decoded_y = Dropout(0.2)(hidden_y)
    # decode
    decoded_x = Dense(input_dim)(hidden_x)
    decoded_y = Dense(input_dim)(hidden_y)
...
# def lstm_ae_2i2o(input_dim, latent_dim, timesteps, l2_reg=0.001, dropout_rate=0.0):
#     # encode
#     inputs_x = Input(shape=(timesteps, input_dim))
#     encoded_x = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid", 
#                      return_sequences=False, kernel_regularizer=l2(l2_reg), 
#                      recurrent_regularizer=l2(l2_reg))(inputs_x)
#     # encoded_x = Dropout(dropout_rate)(encoded_x)

#     inputs_y = Input(shape=(timesteps, input_dim))
#     encoded_y = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid", 
#                      return_sequences=False, kernel_regularizer=l2(l2_reg), 
#                      recurrent_regularizer=l2(l2_reg))(inputs_y)
#     # encoded_y = Dropout(dropout_rate)(encoded_y)

#     # hidden layer after concatenation
#     hidden = Concatenate()([encoded_x, encoded_y])
#     hidden = Dense(latent_dim, kernel_regularizer=l2(l2_reg))(hidden)

#     hidden_x = Dense(latent_dim, kernel_regularizer=l2(l2_reg))(hidden)
#     hidden_y = Dense(latent_dim, kernel_regularizer=l2(l2_reg))(hidden)

#     hidden_x = RepeatVector(timesteps)(hidden_x)
#     hidden_y = RepeatVector(timesteps)(hidden_y)

#     decoded_x = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid", 
#                      return_sequences=True, kernel_regularizer=l2(l2_reg), 
#                      recurrent_regularizer=l2(l2_reg))(hidden_x)
#     # decoded_x = Dropout(dropout_rate)(decoded_x)

#     decoded_y = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid", 
#                      return_sequences=True, kernel_regularizer=l2(l2_reg), 
#                      recurrent_regularizer=l2(l2_reg))(hidden_y)
#     # decoded_y = Dropout(dropout_rate)(decoded_y)

#     decoded_x = TimeDistributed(Dense(input_dim, kernel_regularizer=l2(l2_reg)))(decoded_x)
#     decoded_y = TimeDistributed(Dense(input_dim, kernel_regularizer=l2(l2_reg)))(decoded_y)

#     model = Model([inputs_x, inputs_y], [decoded_x, decoded_y])
#     model.summary()
#     return model

def lstm_ae_simple(input_dim, latent_dim, timesteps, l2_reg=0.001, dropout_rate=0.0):
    # encode
    inputs_x = Input(shape=(timesteps, input_dim))
    # encoded_x = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid", 
    #                  return_sequences=False, kernel_regularizer=l2(l2_reg), 
    #                  recurrent_regularizer=l2(l2_reg))(inputs_x)
    encoded_x = Dense(latent_dim, activation="relu", kernel_regularizer=l2(l2_reg))(inputs_x)
    # encoded_x = Dropout(dropout_rate)(encoded_x)

    inputs_y = Input(shape=(timesteps, input_dim))
    encoded_y = Dense(latent_dim, activation="relu", kernel_regularizer=l2(l2_reg))(inputs_y)
    # encoded_y = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid", 
    #                  return_sequences=False, kernel_regularizer=l2(l2_reg), 
    #                  recurrent_regularizer=l2(l2_reg))(inputs_y)
    # encoded_y = Dropout(dropout_rate)(encoded_y)
    ...
    # hidden layer after concatenation
    hidden = Concatenate(axis=2)([encoded_x, encoded_y])
    hidden = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid",
                    return_sequences=False, kernel_regularizer=l2(l2_reg), 
                    recurrent_regularizer=l2(l2_reg))(hidden)
    ...
    hidden = RepeatVector(timesteps)(hidden)
    decoded = LSTM(latent_dim, activation="tanh", recurrent_activation="sigmoid",
                    return_sequences=True, kernel_regularizer=l2(l2_reg),
                    recurrent_regularizer=l2(l2_reg))(hidden)
    decoded_x = TimeDistributed(Dense(input_dim, kernel_regularizer=l2(l2_reg)))(decoded)
    decoded_y = TimeDistributed(Dense(input_dim, kernel_regularizer=l2(l2_reg)))(decoded)
