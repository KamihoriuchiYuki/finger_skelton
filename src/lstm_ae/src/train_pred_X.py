...
params = {
    ...
    "batch_size": 20,
    "missing_rate": 0.0,
    "data_path": "~/Sensor-Glove/src/data_handler/data/ind_0810_1225.csv"
}
...
XY_train_zero_filtered = XY_train_zero[mask_train]
XY_test_zero_filtered  = XY_test_zero [mask_test]
...
# make model
model=lstm_model.lstm_predXY(input_dim, latent_dim, timesteps)
model.summary()
model.compile(optimizer='adam', loss=lstm_model.masked_mse_loss)
history = model.fit(XY_train_zero, XY_train_last, epochs=epochs, batch_size=batch_size, shuffle=True, validation_data=(XY_train_zero, XY_train_last))


# predict with model
XY_hat_train = model.predict(XY_train_zero)
XY_hat_test  = model.predict(XY_test_zero)


# save
params['train_size'] = XY_train_zero.shape[0]
